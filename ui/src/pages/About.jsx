...
const CERTS = [
  { icon: <VerifiedIcon sx={{ fontSize: 36, color: '#1B6CA8' }} />, title: 'BIS Certified', desc: 'Bureau of Indian Standards' },
  { icon: <VerifiedIcon sx={{ fontSize: 36, color: '#2ECC71' }} />, title: 'FSSAI Licensed', desc: 'Food Safety & Standards Authority' },
  { icon: <VerifiedIcon sx={{ fontSize: 36, color: '#C9A84C' }} />, title: 'NABL Tested', desc: 'National Accreditation Board' },
  { icon: <VerifiedIcon sx={{ fontSize: 36, color: '#4FC3F7' }} />, title: 'ISO 9001:2015', desc: 'Quality Management System' },
];
            ...
            {CERTS.map((c, i) => (
              <Grid item xs={6} md={3} key={c.title}>
                <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                  <Box sx={{ textAlign: 'center', p: 4, background: '#F8FBFF', borderRadius: 4, border: '1px solid #E2EAF4' }}>
                    <Box sx={{ mb: 2 }}>{c.icon}</Box>
                    <Typography sx={{ fontWeight: 700, color: '#0A2342', fontSize: '1rem', mb: 0.5 }}>{c.title}</Typography>
                    <Typography sx={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{c.desc}</Typography>
                  </Box>
                </motion.div>
              </Grid>
            ))}
